...
export const clearHighlight = (keys: string[] | string) => {
  ...
  const keysArray: Array<string> = Array.prototype.concat(keys);
...
export const highlightRegExp = (
  ...
  const tw = makeTreeWalker(options.excludes, options.root);

  const groupedNodes = makeGroupedNodeArray(tw, options.root);
  ...
  const nodeList: Node[][] = [];
  const groupedNodesLength = groupedNodes.length;
  ...
  for (
    ...
    const curGroupOfNodes = groupedNodes[i];
  ...
  for (
    ...
    if (match.percent >= 0.75) {
      ...
      if (nodeStartIndex + match.size > curGroupOfNodes[j].data.length) {
        // get the string that starts at the found match
        // and ends at the end of the containing nodes text
        const inThisNode = nodeParts.substring(match.index);
...
const replacePartOfNode = (
  ...
  const newNode = nodeToReplace.splitText(startOfReplace);
  ...
  const insertedNode = newNode.parentNode?.insertBefore(replaceWith, newNode);
  if (insertedNode != null && insertedNode.firstChild instanceof Text) {
    const insertedText = insertedNode.firstChild;
...
const insertOverSeveralNodes = (
  ...
  for (let k = 0; helpArr.join('').length < fullMatchLength; k++) {
    ...
    const { insertedNode, insertedText } = replacePartOfNode(
...
const findClosestMatch = (str1: string, str2: string): ClosestMatch => {
  ...
  j += 1;
  const nStr2 = str2.substring(0, j);
  ...
  const mat2 = lev_distance_matrix(rStr1, rStr2);
  for (
  ...
  i += 1;
  i = rStr2.length - i;
  const changes = lev_distance(str1, str2.substring(i, j));
  const match: any = [];
